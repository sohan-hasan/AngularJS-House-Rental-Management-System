...
namespace HouseRentalManagementSystem.Areas.User.Controllers
{
    ...
    public class ApartmentController : Controller
    {
        ...
        public JsonResult GetAll()
        {
            ...
            return Json(apartment, new JsonSerializerOptions());

        }
        [HttpGet]
        public JsonResult GetAllType()
        {
            var types = iApartmentTypeRepository.GetAll();
            return Json(types, new JsonSerializerOptions());
        ...
        {
            var types = iApartmentFacilityRepository.GetAll();
            return Json(types, new JsonSerializerOptions());

        }

        [HttpGet]
        public JsonResult GetApartmentImages(int id)
        {
            var images = iApartmentImageRepository.GetAllByApartmentId(id);
            return Json(images, new JsonSerializerOptions());

        }
        [HttpPost]
        public JsonResult Create([FromBody] ApartmentViewModel objModel)
        {
            ApartmentViewModel returnObj = new ApartmentViewModel();
            if (ModelState.IsValid)
            {
                 returnObj = iApartmentRepository.Insert(objModel);
            }
            return Json(returnObj.AptId, new JsonSerializerOptions());
        }
        [HttpPost]
        public JsonResult Edit([FromBody] ApartmentViewModel objModel)
        {
            ApartmentViewModel returnObj = new ApartmentViewModel();
            if (ModelState.IsValid)
            {
                returnObj = iApartmentRepository.Update(objModel);
            }
            return Json(returnObj.AptId, new JsonSerializerOptions());
        }
        [HttpPost]
        public JsonResult DeleteImage(int id)
        {
            ...
            var images = iApartmentImageRepository.GetAllByApartmentId(image.AptId);
            return Json(images, new JsonSerializerOptions());
        }
